...
export default function StaffManagementPage() {
    ...
    const handleToggleStatus = async (staff: Staff) => {
        try {
            ...
            setStaff(prev =>
            prev.map(s =>
                s._id === staff._id ? { ...s, isActive: !s.isActive } : s
            )
            );
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            setStaff(prev =>
            prev.filter(s => s._id !== deletingStaff._id)
            );
        ...
        {filteredStaff.map((staffMember) => (
            ...
            <Card className="hover:shadow-lg transition-shadow">
                ...
                <div className="flex items-start justify-between mb-3">
                    ...
                    <Chip
                    size="small"
                    label={staffMember.isActive ? 'Đang làm' : 'Nghỉ việc'}
                    color={staffMember.isActive ? 'success' : 'default'}
                    icon={staffMember.isActive ? <Check /> : <Close />}
                    />
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    >
                    {staffMember.isActive ? 'Tạm nghỉ' : 'Kích hoạt'}
                    </Button>
                    <IconButton
                    size="small"
                    color="error"
