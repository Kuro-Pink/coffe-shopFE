...
import { Staff, StaffStats, StaffPerformance, StaffType, Order } from '@/types';
...
export const staffService = {
    ...
    getPerformance: async (
        ...
        API_ENDPOINTS.HOST.STAFF.PERFORMANCE(storeId),
        { params }
        );
        return response.data.data;
    },
...
export const staffOrderService = {
    ...
    getOrderById: async (id: string): Promise<Order> => {
        const response = await api.get(API_ENDPOINTS.STAFF.ORDER_DETAIL(id));
        return response.data.data;
    },
